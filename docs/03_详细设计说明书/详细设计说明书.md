...
**并发能力定义**：演示级 ≥5 并发指"5 个用户可同时提交任务"，但算法推理**串行排队**，总耗时 = 单张耗时 × 任务总数。

#### 3.4.1 并发瓶颈分析

| 层 | 并发行为 | 瓶颈 |
|---|---|---|
| 前端 | 5 个用户同时点击"开始检测" | 无瓶颈 |
| 后端 API | FastAPI 异步处理，立即返回 `taskId` | 无瓶颈（仅创建任务记录） |
| 算法服务 | YOLOv8 单线程推理，多请求排队 | **核心瓶颈** |
| 数据库 | 连接池默认 5，足够演示级 | 可扩展到 10+ |
| 文件写入 | 标注图按 `{uuid}` 命名，无冲突 | 无瓶颈 |

#### 3.4.2 轻量级任务队列（推荐实现）

使用 Python `asyncio.Queue` 实现算法服务内部排队，无需引入 Celery/Redis：
...
app = FastAPI()
infer_queue = asyncio.Queue(maxsize=10)  # 最多排队10个任务
infer_lock = asyncio.Lock()              # 推理互斥锁

@app.post("/infer")
async def infer(request: InferRequest):
    # 请求入队
    await infer_queue.put(request)
    queue_position = infer_queue.qsize()
    
    # 等待推理（串行执行）
    async with infer_lock:
        # 从队列取出
        current = await infer_queue.get()
        
        # 执行推理（同步代码包裹在 run_in_executor）
        result = await asyncio.to_thread(pipeline.run, current.image_path, current.standard_id)
...
- 无外部依赖，部署简单
- 自动排队，避免模型并发冲突
- 请求阻塞等待，前端轮询 `GET /tasks/{id}` 查看进度

**折衷**：
- 5 个并发请求总耗时约 15 秒（3秒 × 5），用户需等待
- 超过 10 个排队任务会被拒绝（返回 503），演示场景可接受

#### 3.4.3 数据库并发安全

**`detection_task.done_count` 原子更新**：
...
# 正确写法（原子更新）
db.query(DetectionTask).filter_by(id=task_id).update({
    "done_count": DetectionTask.done_count + 1
})
db.commit()
```
...
engine = create_engine(
    DATABASE_URL,
    pool_size=10,          # 默认5改为10
    max_overflow=5,        # 峰值15个连接
...
- **进度反馈**：前端轮询 `GET /tasks/{id}` 显示 `done_count / total_count`
- **队列位置提示**（可选）：算法服务返回 `queue_position` 告知"前方还有 N 个任务"
- **超时处理**：单张推理超过 10 秒视为超时，触发降级兜底
...
| 项 | 内容 |
|---|---|
| 请求体 | `{ reviewed_grade: Grade }` |
| 行为 | 管理员人工修正等级，`is_reviewed=1` |

#### GET /results/{id}/explain（AI 智能解释，P1 加分项）

| 项 | 内容 |
|---|---|
| 鉴权 | 是 |
| 功能 | 调用多模态大模型（Qwen-VL/GPT-4V）生成缺陷成因、严重程度、分级依据、储存建议的通俗文字说明 |
| 响应 data | `{ explanation: str, llm_model: str, generated_at: datetime }` |
| 说明 | 异步生成（约 3~15 秒），失败返回默认文本；需配置 `LLM_ENABLED=true` 启用 |
...
```json
{
  ...
  "msg": "ok",
  "data": {
    "explanation": "这个柑橘被判定为二级（合格品）。图中左上角有一处中等大小的病斑，可能是黑星病引起，置信度82%。着色率达到73%，略高于二级标准（70%），但缺陷占比接近4%，未达到一级要求（<2%）。果径48mm符合标准。建议：该果品适合近期销售，不宜长期储存，因病斑可能扩散。",
    "llm_model": "qwen-vl-max",
    "generated_at": "2026-06-23T10:30:45"
  }
}
```
|---|---|
...
| LLM 服务超时 | 超时 / 降级 | "智能解释服务暂时不可用" |

---

## 7.4 多模态大模型增强模块（LLM，P1 加分项）

### 7.4.1 设计目标

为检测结果增加**智能文字解释**功能，调用多模态大模型（Qwen-VL / GPT-4V / Claude）生成通俗易懂的缺陷分析、分级依据、储存建议，提升用户可理解性与教育价值。

**设计原则**：
- **可选 + 异步**：不阻塞主检测流程，失败不影响核心功能
- **厂商中立**：采用 OpenAI 兼容接口，一套代码适配任意兼容模型
- **本地兜底**：支持 Ollama 本地部署，演示时离线可用
...
class LLMClient:
    """OpenAI 兼容 LLM 客户端，支持云端 API + 本地 Ollama"""
    ...
    def explain_defects(self, annotated_image_path: str, result: DetectionResult) -> str:
        """
        生成缺陷智能解释
        ...
        try:
            ...
            return self._get_fallback_message(result)
    
    def _build_prompt(self, result: DetectionResult) -> str:
        """构建 Prompt，注入检测结果结构化数据"""
        defect_list = [f"{d.type}（置信度{d.confidence:.2f}）" 
                       for d in result.defects] or ["无明显缺陷"]
        
        return f"""你是柑橘品质专家。根据标注图片和检测数据，用通俗语言为果农或质检员解释：

1. **缺陷分析**：各缺陷的类型、严重程度、可能成因
2. **分级依据**：为什么被判定为该等级（结合量化指标）
3. **实用建议**：储存、运输、销售的注意事项

检测结果：
- 等级：{result.grade_display}
- 果径：{result.diameter_mm} mm
- 着色率：{result.color_ratio}%
- 缺陷占比：{result.defect_ratio}%
- 检出缺陷：{', '.join(defect_list)}

要求：限 200-300 字，语言通俗，避免专业术语，重点突出用户关心的"为什么"和"怎么办"。"""
    
    def _get_fallback_message(self, result: DetectionResult) -> str:
        """LLM 不可用时的降级文本"""
        grade_desc = {"grade1": "一级（精品）", "grade2": "二级（合格）", "out": "等外品"}
        return f"""系统已完成自动检测，判定为{grade_desc[result.grade]}。
果径 {result.diameter_mm}mm，着色率 {result.color_ratio}%，缺陷占比 {result.defect_ratio}%。
详细指标请参考上方数据卡，标注图已框选出检测到的缺陷区域。"""


# Dependency Injection
def get_llm_client() -> Optional[LLMClient]:
    """获取 LLM 客户端（若未启用则返回 None）"""
...
async def explain_result(
    ...
    """生成检测结果的智能解释（AI 增强功能）"""
    # 1. 查询结果
    result = db.query(DetectionResult).filter_by(id=id).first()
    if not result:
    ...
    if not llm:
        return {
            "explanation": llm._get_fallback_message(result),
            "llm_model": "fallback",
...
**backend/.env**（新增 LLM 配置块）：
```bash
# ========== LLM 智能解释（可选，P1 加分项） ==========
LLM_ENABLED=true

# 云端模式（推荐：通义千问 Qwen-VL，中文强 + 多模态）
LLM_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
LLM_API_KEY=sk-your-dashscope-api-key
LLM_MODEL=qwen-vl-max

# 本地模式（演示兜底：Ollama，离线可用）
# LLM_BASE_URL=http://localhost:11434/v1
# LLM_API_KEY=ollama  # 本地无需真实 key，填任意值
# LLM_MODEL=qwen2.5-vl:7b

LLM_TIMEOUT=15
LLM_FALLBACK_MESSAGE="智能解释服务暂时不可用，请参考检测指标"
```
...
<template>
  <div class="result-detail">
    ...
    <div class="ai-explanation-card">
      <button @click="fetchExplanation" :disabled="loading">
        <IconSparkles /> 智能解释
      </button>
      
      <div v-if="loading" class="loading">
        <Spinner /> AI 分析中，约需 5-10 秒...
      </div>
      
      <div v-if="explanation" class="explanation-text">
        <IconLightbulb />
        <p>{{ explanation }}</p>
        <small>由 {{ llmModel }} 生成</small>
      </div>
    </div>
  </div>
</template>
...
const fetchExplanation = async () => {
  loading.value = true
  try {
    const res = await api.get(`/results/${props.resultId}/explain`)
    explanation.value = res.data.explanation
    llmModel.value = res.data.llm_model
  } catch (err) {
    explanation.value = "智能解释服务暂时不可用"
  } finally {
    loading.value = false
  }
}
...
| 模式 | 响应时间 | 单次成本 | 说明 |
|---|---|---|---|
| 云端 Qwen-VL-Max | 3~8 秒 | ~¥0.01 | 效果最佳，中文理解强 |
| 云端 GPT-4V | 5~10 秒 | ~¥0.03 | 效果好但贵，英文偏向 |
| 本地 Ollama (qwen2.5-vl:7b) | 5~15 秒 | 免费 | 离线可用，效果略弱但够用 |

**课设成本估算**：演示 50 次 × ¥0.01 = **¥0.5**，可忽略。

### 7.4.7 风险与应对

| 风险 | 应对 |
|---|---|
| LLM 输出不可控（偶尔胡言） | Prompt 约束格式 + 后验检查长度（100~500 字） |
| 云端限流或欠费 | 优雅降级到预设文本，不报错 |
| 答辩现场网络差 | 提前切换到本地 Ollama，演示前测试 |
| 响应慢用户等待 | 异步加载 + 进度提示"AI 分析中..." |
