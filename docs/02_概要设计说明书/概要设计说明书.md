...
当前数据库设计已满足课设演示需求，但存在以下可优化点（生产环境建议改进）：

| 限制 | 影响 | 优化建议 | 优先级 |
|---|---|---|---|
| `detection_result.grade` 无索引 | 统计看板按等级分组查询时，大数据量会全表扫描 | 增加 `INDEX idx_grade (grade)` | 低（演示数据量小） |
| `grade_standard.is_active` 无唯一约束 | 可能出现多个激活标准 | 应用层事务控制 + 触发器强制唯一 | 中（需业务层保证） |
| `price_reference.is_active` 同上 | 同一等级可能有多个有效价格 | 同上 | 中 |
| `traceability` 缺时间查询索引 | 按检测时间范围查询溯源记录时性能差 | 增加 `INDEX idx_detect_time (detect_time)` | 低 |

**实施建议**：演示阶段保持现状；若投入生产使用，需在应用层的 `StandardService.activate()` 和 `PriceService.update()` 方法中用事务确保单一激活。
